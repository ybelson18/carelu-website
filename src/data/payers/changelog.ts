...
export const PAYER_CHANGELOG: PayerChangeEntry[] = [
  {
    ...
    type: 'correction',
    summary:
      'The three facts rendered at the top of every guide page \u2014 assessment prior auth, treatment prior auth, and whether an autism diagnosis is required \u2014 were bare strings with no status and no citation. That is precisely how a wrong claim survived on six Virginia guides for two months: "source-verified" lived in a commit message instead of in the data, so nobody could audit it, including us. All 579 are now PayerRuleFact like every other field, and the type no longer permits a value without a status. 72% of the directory\'s 2,872 structured facts are now verified against a primary source, up from a state where 579 of them carried no evidence at all; about 60% of that was extraction \u2014 citations already sitting in the guides\' own prose, never attached to a field. Non-verified facts are now split by WHY: 292 \'document\' (the answer is written down and we could not open the file \u2014 real, closeable debt), 489 \'per-case\' (no document states it because it varies by plan or member, so "ask the plan" is the finished answer, not a placeholder), and 9 \'licensed\' (proprietary MCG/InterQual criteria that are never published and that no document request will ever satisfy). The page says which, so a permanent correct answer stops looking like an unpaid task. CORRECTIONS: 56 citations across every Aetna and Cigna guide named the wrong document \u2014 Aetna CPB 0554 contains no precertification language at all (it is the exclusion bulletin, and its own code table reads "CPT codes NOT covered ... 97151-97158"), and Evernorth EN0499 states no prior-authorization rule; both facts were right, both attributions were wrong, and the real sources are Aetna\'s BH precertification list and Cigna\'s autism resource guide. Five guides asserted that Evernorth publishes no supervision ratio when EN0499 publishes one verbatim ("one to two hours per ten hours of direct treatment"). Three more North Carolina guides still carried the flat 180-day authorization period after the first correction reached only two. AND A CORRECTION TO A CORRECTION: the Virginia diagnosis fix shipped earlier the same day was itself partly wrong, by the exact mechanism it was fixing \u2014 an extraction regex took the FIRST match of "primary ICD diagnosis" in a multi-service appendix, which belongs to Functional Family Therapy, and imported FFT\'s diagnostic-category list into the ABA criterion, which has no category list at all. The real ABA rule is broader than the "correction" claimed. Caught by a later agent, re-verified by counting occurrences and locating each one\'s parent section, fixed on all six guides. New tooling: `npm run payers:check` exits non-zero unless every fact has a status, every verified fact cites a source, and every non-verified fact carries both a verifyVia and a blocker \u2014 it immediately caught 118 facts whose verifyVia had been silently dropped by a patch-applier bug, which no report would have surfaced. Also recorded: `npx tsc --noEmit` is a NO-OP in this repo (the root tsconfig has files: []), and it reported clean against 583 real type errors.',
    details: [
      {
        slug: 'all 193 guides',
        field: 'assessmentPA, treatmentPA, dxRequired',
        change:
          'Migrated from bare `string` to `PayerRuleFact` (value + status + cites + verifyVia + blocker). 579 facts. The schema now refuses a value without a status, closing the failure mode that produced the Virginia error. Consumers updated: the top prior-auth panel, the directory card chips (which now render only for a VERIFIED fact, so an unverified guess cannot look like an answer), and /api/ask, which receives the status inline so the assistant cannot present an unverified field as settled.',
      ...
      {
        slug: 'virginia-medicaid + 5 Cardinal Care MCOs',
        field: 'dxRequired',
        change:
          'CORRECTION TO THE SAME DAY\u2019S CORRECTION. DMAS Appendix D contains exactly two occurrences of "primary ICD diagnosis": one under Functional Family Therapy (p.14), which carries the "disruptive behaviour, mood, substance use or trauma and stressor-related" category list, and one under ABA (p.24), which carries NO category list and instead reads "...correlates to a DSM diagnosis or has a provisional psychiatric diagnosis as developed by an LMHP when no definitive diagnosis has been made". The earlier fix matched the first occurrence and narrowed the ABA benefit with FFT\u2019s categories. Virginia ABA is neither autism-restricted nor category-restricted. The supervision correction from the same pass was re-checked the same way (both quotes unique, both in the ABA section) and stands.',
        sourceUrl: 'https://vamedicaid.dmas.virginia.gov/sites/default/files/2025-07/MHS%20-%20Appendix%20D%20(updated%207.17.25)_Final.pdf',
      },
      {
        slug: 'all aetna-* and cigna-* guides (56 strings)',
        field: 'assessmentPA, treatmentPA, cites, sources',
        change:
          'Aetna CPB 0554 was read in full: zero occurrences of "precertification", "prior authorization" or "GR-69017". Evernorth EN0499 was read in full: no prior-authorization statement anywhere. Citations and value wording both re-pointed to Aetna\u2019s Participating provider behavioral health precertification list (eff. 8/1/2024) and Cigna\u2019s autism resource guide (March 2025). EN0499 remains the correct source for the diagnosis criteria it does state.',
        sourceUrl: 'https://static.cigna.com/assets/chcp/pdf/coveragePolicies/medical/autism-resource-guide.pdf',
      },
      {
        slug: 'cigna-florida, cigna-georgia, cigna-massachusetts, cigna-nebraska, cigna-new-jersey',
        field: 'deliveryRules.supervision',
        change:
          'CORRECTION \u2014 the guides said Evernorth publishes no supervision floor. EN0499 "Supervision / Direction of Treatment" publishes one: direct plus indirect case supervision "is consistent with the general accepted standard of care of one to two hours per ten hours of direct treatment", and "when direct treatment is 10 hours per week or less, a minimum of one to two hours per week of direct case supervision is provided."',
  ...
  },
  {
    ...
    type: 'policy-update',
    summary:
      'Field-completeness pass across all 193 guides, plus five corrections to live pages that it turned up. An audit showed the directory was rich for a few payers and thin for the rest: delivery-and-billing rules sat at 15-20% coverage, and the intake-side questions that decide whether a family can START were captured nowhere structured at all. A new `intakeGates` group fixes that — age limit, diagnosis recency, who may diagnose, required diagnostic tools, referral, and telehealth — and both groups are now filled across every guide: intake gates 0% to 100%, delivery rules 15-20% to 95-99%, and assessmentPA/treatmentPA/dxRequired to 100%. 2,293 structured facts now exist where there were roughly 250. Method mattered as much as the result: a third to a half of these facts already sat in the guides as cited prose, so the first pass was extraction — lifting a fact into a field carrying the citation it already had — and only what prose did not cover was researched fresh. Managed-care plans inherit their state floor only where an agent confirmed the plan publishes nothing of its own; where that could not be checked, the field says unverified and names who to call rather than assuming. 70% of the 2,293 facts are verified against a primary source, 6% are plan-dependent, and 24% are unverified with a named portal or phone line — that last number is deliberate and visible, not a gap being papered over. FIVE CORRECTIONS. Virginia: dxRequired read "ASD diagnosis by a qualified professional" on the state guide and all five Cardinal Care MCOs, but DMAS Appendix D contains ZERO occurrences of the word autism — admission turns on a youth under 21 with a primary ICD/DSM diagnosis in the disruptive-behaviour, mood, substance-use or trauma-and-stressor categories, or a provisional psychiatric diagnosis from an LMHP when none is definitive. The directory was telling Virginia intake teams to turn away eligible families. Same file: prose claimed DMAS sets no supervision frequency, when Appendix D requires monthly observation of the youth and twice-monthly supervision of unlicensed staff. North Carolina: a blanket "up to 180 days" authorization period, when CCP 8F is tiered — 180 days at 16 hrs/week or fewer, but only 90 days above that, which is most comprehensive programmes. Kansas: KMAP Bulletin 26140 was reissued 8/31/2026 and its effective date moved from September to November 1, 2026 — corrected in 19 places. New York: one surviving "680 hours/year" in a meta description, and the same repealed cap still live in the VOB layer, where a shared helper was stamping it onto multiple entries and the DFS circular was labelled as its authority — that layer is vendored into the product, so the website fix alone would have left the product wrong. Fourteen further challenged claims are logged in docs/payer-worklist.json rather than applied, including two on anthem-bcbs-georgia where the cited PDF was read in full and does not contain the facts attributed to it. New tooling: scripts/payer-coverage.mjs reports per-field coverage AND the verified/unverified ratio; the monthly refresh now runs it at both ends of every cycle, may not let any field group regress, and must fill a new guide\'s fields in the same cycle it is added. The runbook\'s source-access table grew to 21 hosts, and now carries three habits learned the hard way: an HTTP 200 is not proof you got the document (medicaid.georgia.gov serves a real PDF that is the wrong document), a WebFetch "not mentioned" on a PDF is not evidence of absence (image-heavy PDFs defeat the summariser — download and pdftotext before concluding a rule does not exist), and a source you could not read is a document request, never a guess. Two unblocks worth keeping: curl against web.archive.org/web/2026id_/ recovers documents from hosts that refuse us outright, and appending ?__tracked=1 to Optum DAM URLs defeats their download interstitial.',
    details: [
      {
        slug: 'virginia-medicaid, aetna-better-health-virginia, anthem-healthkeepers-plus, humana-healthy-horizons-virginia, sentara-community-plan, unitedhealthcare-community-plan-virginia',
        field: 'dxRequired, sections (supervision)',
        change:
          'CORRECTION — Virginia Medicaid ABA is a youth behavioural-health benefit, NOT an autism benefit. DMAS Appendix D (rev. 7/17/2025) has zero occurrences of "autism": admission requires a youth under 21 who "currently meets criteria for a primary ICD diagnosis that correlates to a DSM diagnosis" in the disruptive-behaviour, mood, substance-use or trauma-and-stressor categories, "or has a provisional psychiatric diagnosis as developed by an LMHP when no definitive diagnosis has been made", plus at least two functional-impairment criteria. Also corrected: the claim that DMAS defers all supervision frequency to the Board and BACB — Appendix D requires the LBA/LABA/LMHP to observe the youth and supervise monthly, and supervision of unlicensed staff at least twice a month, each evidenced by a contemporaneously dated signature.',
        sourceUrl: 'https://vamedicaid.dmas.virginia.gov/sites/default/files/2025-07/MHS%20-%20Appendix%20D%20(updated%207.17.25)_Final.pdf',
      },
      {
        slug: 'north-carolina-medicaid, healthy-blue-north-carolina',
        field: 'cardDesc, metaDescription, atGlance, treatmentPA, sections',
        change:
          'CORRECTION — authorization length is tiered, not flat. CCP 8F 5.1: "For Treatment Plans involving 16 hours or fewer services per week per beneficiary, Medicaid covers up to 180-calendar days for the initial authorization period"; "For Treatment Plans involving more than 16 hours of services per week, Medicaid covers up to 90 calendar days". The guides stated a blanket 180 days, which is wrong for most comprehensive programmes — they reauthorize quarterly, not twice a year.',
      ...
      },
      {
        ...
        field: 'dxRequired, atGlance, sections, faq, sources (19 occurrences)',
        change:
          'CORRECTION — KMAP General Bulletin 26140 was reissued 8/31/2026 and its effective date moved from September 1 to November 1, 2026 (the published page shows "September" struck through). Separately logged as a challenged claim: the bulletin states that once diagnosed, members are eligible "without the need for a re-evaluation" and with "no time limit on how soon they begin autism treatment", which contradicts the guides\' repeated 6-month diagnosis-validation rule.',
        sourceUrl: 'https://www.sunflowerhealthplan.com/newsroom/kmap-26140.html',
      },
      {
        slug: 'unitedhealthcare-new-york + vob/new-york',
        field: 'metaDescription, vob codeGrid notes and source labels',
        change:
          'CORRECTION — the repealed 680-hour cap survived in two places the first pass missed: one meta description, and the VOB enrichment layer, where a shared nyMandateNote() helper stamped "caps ABA at 680 hours per policy/calendar year" onto multiple entries and a source label credited DFS Circular Letter 6 as its authority. The VOB layer is vendored into the product and read at runtime, so correcting the guides alone would have left the product wrong. MetroPlus\'s Child Health Plus benefit limit is documented separately in that plan\'s own manual and was deliberately left intact.',
  ...
  },
  {
    ...
    type: 'guides-added',
    summary:
      'Seven states added: 73 guides, directory 193 -> 266 guides across 26 states. NEW: Idaho (6), California (16), Michigan (12), Pennsylvania (14). PORTED from the LeadTrap product and completed: Hawaii (9), Iowa (8), Oklahoma (8) - those files existed only in LeadTrap, lacked every structured field, and a month-old re-verification found 35 stale facts between them (e.g. Oklahoma diagnosing-provider list and 2-year evaluation recency per OAC 317:30-5-313 rev. 9/1/2025; Iowa rule renumbering and H.F. 330 removing mandate caps from 1/1/2026; Hawaii QI-2431 read in full and Ohana Health Plan exiting QUEST 12/31/2026). Every new guide carries all 17 structured facts (assessment/treatment PA, diagnosis, 6 delivery rules, 8 intake gates) under the verified-only contract; each commercial guide carries its state autism-mandate rows. Shape notes intake teams need: Idaho Medicaid pays ABA as CHIS on HCPCS codes (Magellan stopped 97151-97158 on 12/1/2025), no diagnosis required, and Idaho has neither an autism statute (DOI Bulletin 18-02 is the floor) nor behavior-analyst licensure; Medi-Cal BHT is for under-21 regardless of diagnosis via the county Medi-Cal plan, with Regional Centers paying last, and California has no behavior-analyst license (QASP tiers); Michigan Medicaid ABA is carved out to the PIHP/CMH system, not the Medicaid Health Plans, and the PIHP re-procurement for 10/1/2026 is unconfirmed; Pennsylvania MA pays ABA as IBHS through the county HealthChoices BH-MCO for any BH diagnosis under 21, Act 62 caps at \$51,908 (2026, CPI-adjusted) for large-group insured plans, and PH-95 keeps MA open as secondary regardless of parental income. Two template issues in existing Optum and Aetna guides surfaced during the build and are logged as VC-039 and VC-040 rather than applied unverified.',
    guides: [
      'idaho-medicaid',
      'blue-cross-of-idaho',
